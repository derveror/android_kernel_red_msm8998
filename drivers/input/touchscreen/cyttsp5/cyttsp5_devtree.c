...
/* Virtual keys are intentionally not part of the RED JDI topology. */
...
static const char *device_names[DEVICE_TYPE_MAX] = {
	[DEVICE_MT] = CYTTSP5_MT_NAME,
	[DEVICE_BTN] = CYTTSP5_BTN_NAME,
	[DEVICE_PROXIMITY] = CYTTSP5_PROXIMITY_NAME,
};
		...
		enum cyttsp5_device_type *type)
{
	...
	rc = of_property_read_string(dev_node, "cy,name", &name);
	if (rc) {
		tp_log_err("%s %d:[cy,name] read fail\n", __func__, __LINE__);
		return rc;
	}
...
/* Parse one optional touch-settings array from devicetree. */
...
static struct cyttsp5_power_config *create_and_get_power_config(struct device_node *core_node)
{
	...
	power_config->vdd_type = CY_POWER_NONE;
	power_config->vbus_type = CY_POWER_NONE;
...
/* Parse the RED Cypress core node and construct its platform data. */
		...
		struct device_node *core_node)
{
	...
	/* RED stock ABI stores TLMM numbers as raw u32 values. */
	...
	pdata->input_register_early = get_of_u32_val(core_node,
			"cy,input_register_early", 0);
	tp_log_info("%s:input_register_early = %d\n", __func__,
			pdata->input_register_early);
...
static void free_core_pdata(void *pdata)
{
	...
	if (!IS_ERR_OR_NULL(core_pdata->wakeup_keys)) {
		kfree(core_pdata->wakeup_keys->keys);
		kfree(core_pdata->wakeup_keys);
	}
	kfree(core_pdata->power_config);
	kfree(core_pdata);
}
...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
		...
		rc = of_property_read_string(core_node, "cy,name", &name);
		if (!rc) {
		...
		pdata->loader_pdata = NULL;
	}

	pr_info("%s: %d child node(s) found\n", __func__, count);
	if (rc) {
		kfree(pdata);
		adap_dev->platform_data = NULL;
	}
