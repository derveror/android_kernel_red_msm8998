						...
						struct snd_soc_dai *dai)
{
	...
	/* SmartAmp only supported 48kHz will cause voice call speaker no sound.
	 * Remove snd_pcm_hw_constraint_list() to avoid pcm_open fail with invalid hw parameters. */
	return 0;
		...
		struct device_node *np)
{
	...
	if (!of_property_read_string(np, "tfa98xx,fw-name", &dt_fw_name)) {
		fw_name = (char *)dt_fw_name;
		dev_info(dev, "using firmware container %s\n", fw_name);
	}
